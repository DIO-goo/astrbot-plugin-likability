...
class ContractSystem(Star):
    def __init__(self, context: Context, config: AstrBotConfig):
        ...
        self.config = config
        # 初始化好感度系统
        self.likability_system = LikabilitySystem(config)
        self._init_env()
...
#免责声明：代码中的称呼与词汇为娱乐性质，不涉及政治等敏感内容
    ...
    @filter.command("rp")
    @filter.event_message_type(filter.EventMessageType.GROUP_MESSAGE)
    async def rp_command(self, event: AstrMessageEvent):
        """好感度抽卡命令"""
        user_id = event.message_obj.sender.user_id
        group_id = str(event.message_obj.group_id)
        
        # 调用好感度系统的抽卡功能
        result = self.likability_system.rp_draw(user_id)
        ...
        if result["success"]:
            ...
            # 构建响应消息
            response = f"今天{user_name}带来什么好吃的呢？\n"
            response += f"{result['result']}。{result['message']}\n"
            response += f"当前好感度：{result['current_likability']}/{result['max_likability']}"
            
            yield event.plain_result(response)
        else:
            yield event.plain_result(result["message"])
    
    @filter.command("likabilityGet")
    @filter.event_message_type(filter.EventMessageType.GROUP_MESSAGE)
    async def get_likability_command(self, event: AstrMessageEvent):
        """查询好感度命令"""
        user_id = event.message_obj.sender.user_id
        group_id = str(event.message_obj.group_id)
        ...
        # 调用好感度系统的查询功能
        info = self.likability_system.get_likability(target_id)
        
        # 获取用户昵称
        user_name = await self._get_at_user_name(event, target_id)
        
        # 构建响应消息
        response = f"当前好感度为{info['current_likability']}/{info['max_likability']}，{info['level']}。已经与你相伴{info['total_sign_days']}天"
        
        yield event.plain_result(response)
    
    @filter.command("likabilitySet")
    @filter.event_message_type(filter.EventMessageType.GROUP_MESSAGE)
    async def set_likability_command(self, event: AstrMessageEvent):
        """设置好感度命令（仅管理员）"""
        operator_id = event.message_obj.sender.user_id
        group_id = str(event.message_obj.group_id)
        
        # 解析命令参数
        parts = event.message_str.strip().split()
        if len(parts) < 3:
            yield event.plain_result("❌ 格式错误: /likabilitySet 对方uid 调整数值")
            return
        
        target_id = parts[1]
        try:
            change_value = int(parts[2])
        except ValueError:
            yield event.plain_result("❌ 调整数值必须是整数")
            return
        
        # 调用好感度系统的设置功能
        result = self.likability_system.set_likability(operator_id, target_id, change_value)
    ...
    @filter.command("setUserNickname")
    @filter.event_message_type(filter.EventMessageType.GROUP_MESSAGE)
    async def set_user_nickname_command(self, event: AstrMessageEvent):
        """设置用户昵称命令"""
        user_id = event.message_obj.sender.user_id
        group_id = str(event.message_obj.group_id)
        ...
        parts = event.message_str.strip().split()
        if len(parts) < 2:
            yield event.plain_result("❌ 格式错误: /setUserNickname 昵称")
            return
        
        nickname = " ".join(parts[1:])
        
        # 调用用户画像系统的设置昵称功能
        result = self.likability_system.set_user_nickname(user_id, nickname)
        
        if result["success"]:
            yield event.plain_result(result["message"])
        else:
            yield event.plain_result("设置昵称失败")
    
    @filter.command("setUserPersona")
    @filter.event_message_type(filter.EventMessageType.GROUP_MESSAGE)
    async def set_user_persona_command(self, event: AstrMessageEvent):
        """设置用户印象命令"""
        user_id = event.message_obj.sender.user_id
        group_id = str(event.message_obj.group_id)
        ...
        parts = event.message_str.strip().split()
        if len(parts) < 2:
            yield event.plain_result("❌ 格式错误: /setUserPersona 印象")
            return
        
        impression = " ".join(parts[1:])
        
        # 调用用户画像系统的设置印象功能
        result = self.likability_system.set_user_impression(user_id, impression)
        
        if result["success"]:
            yield event.plain_result(result["message"])
        else:
            yield event.plain_result("设置印象失败")
    
    @filter.command("likabilityBlackList")
    @filter.event_message_type(filter.EventMessageType.GROUP_MESSAGE)
    async def blacklist_command(self, event: AstrMessageEvent):
        ...
        parts = event.message_str.strip().split()
        if len(parts) < 2:
            yield event.plain_result("❌ 格式错误: /likabilityBlackList 对方uid")
            return
        
        target_id = parts[1]
        
        # 调用黑名单功能
        result = self.likability_system.add_to_blacklist(operator_id, target_id)
        ...
        if result["success"]:
            ...
            yield event.plain_result(result["message"])
    #endregion
    def get_likability_prompt_addition(self, user_id: str) -> str:
        """
        获取好感度提示词附加内容
        ...
        Returns:
            提示词附加内容
        """
        return self.likability_system.get_prompt_addition(user_id)
